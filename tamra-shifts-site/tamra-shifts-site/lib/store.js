...
// The three companies this deployment serves. Each gets its own URL path (the slug), its own
// settings/employees/schedules, but shares the same web service + database — see lib/server.js
// for how a request's company is resolved (X-Company-Slug header pre-login, session afterwards).
// 'tamra' keeps the root path ("/") so the company already using this app keeps its existing
// bookmarked link working unchanged.
const DEFAULT_COMPANIES = [
  { id: 'tamra', slug: 'tamra', name: 'תמרה דלקים (96) בע"מ' },
  { id: 'sen-energy', slug: 'sen-energy', name: 'ס.ע.ן אנרגיה בע"מ' },
  { id: 'rms-gold-energy', slug: 'rms-gold-energy', name: 'ר.מ.ס גולד אנרגיה בע"מ' },
];
...
// including the company_id column added to every table when this app became multi-tenant
// (each existing row defaults to company_id='tamra', so the company already using this app
// keeps every bit of its history exactly as it was).
...
async function migrateTimestampColumns(db) {
  ...
  const alters = [
    ...
    "ALTER TABLE shift_templates ADD COLUMN IF NOT EXISTS company_id TEXT NOT NULL DEFAULT 'tamra'",
    "ALTER TABLE schedules ADD COLUMN IF NOT EXISTS company_id TEXT NOT NULL DEFAULT 'tamra'",
    "ALTER TABLE assignments ADD COLUMN IF NOT EXISTS company_id TEXT NOT NULL DEFAULT 'tamra'",
    "ALTER TABLE constraints ADD COLUMN IF NOT EXISTS company_id TEXT NOT NULL DEFAULT 'tamra'",
    "ALTER TABLE availability ADD COLUMN IF NOT EXISTS company_id TEXT NOT NULL DEFAULT 'tamra'",
    "ALTER TABLE swap_requests ADD COLUMN IF NOT EXISTS company_id TEXT NOT NULL DEFAULT 'tamra'",
    "ALTER TABLE notifications ADD COLUMN IF NOT EXISTS company_id TEXT NOT NULL DEFAULT 'tamra'",
    "ALTER TABLE push_subscriptions ADD COLUMN IF NOT EXISTS company_id TEXT NOT NULL DEFAULT 'tamra'",
    // schedules used to be keyed by week_start alone; two companies can now generate the same
    // calendar week, so the primary key must include company_id. Safe to re-run: dropping and
    // re-adding the (already-correct) constraint on a fresh database is a harmless no-op.
    'ALTER TABLE schedules DROP CONSTRAINT IF EXISTS schedules_pkey',
    'ALTER TABLE schedules ADD CONSTRAINT schedules_pkey PRIMARY KEY (company_id, week_start)',
  ];
...
// Ensures the companies table has our three companies, and that each one has a settings row and
// a full set of default shift templates. Runs every startup; idempotent per company (only
// inserts what's missing), so it's a no-op for a company that's already fully set up — which is
// exactly the case for 'tamra' after the migration above backfills its existing data.
async function ensureCompaniesSeeded(db) {
  for (const c of DEFAULT_COMPANIES) {
    const existingCompany = await db.get('SELECT id FROM companies WHERE id = ?', [c.id]);
    if (!existingCompany) {
      await db.run('INSERT INTO companies (id, slug, name, created_at) VALUES (?,?,?,?)', [c.id, c.slug, c.name, Date.now()]);
    }
    const settingsRow = await db.get('SELECT company_id FROM settings WHERE company_id = ?', [c.id]);
    if (!settingsRow) {
      const maxIdRow = await db.get('SELECT COALESCE(MAX(id), 0) as m FROM settings', []);
      const nextId = Number(maxIdRow ? maxIdRow.m : 0) + 1;
      const defaults = Object.assign({}, DEFAULT_SETTINGS, { companyName: c.name });
      await db.run('INSERT INTO settings (id, company_id, data) VALUES (?, ?, ?)', [nextId, c.id, JSON.stringify(defaults)]);
    }
    const tCount = await db.get('SELECT COUNT(*) as c FROM shift_templates WHERE company_id = ?', [c.id]);
    if (!tCount || Number(tCount.c) === 0) {
      for (const t of DEFAULT_TEMPLATES) {
        await db.run(
          'INSERT INTO shift_templates (id, company_id, role_id, label, start_time, end_time, needed, days, active, auto_assign, required_gender, allow_extra) VALUES (?,?,?,?,?,?,?,?,1,?,?,?)',
          [uid(), c.id, t.roleId, t.label, t.start, t.end, t.needed, JSON.stringify(t.days), t.autoAssign === false ? 0 : 1, t.requiredGender || null, t.allowExtra ? 1 : 0]
        );
      }
    }
  }
...
// Scoped to 'tamra' only: it's a one-time patch for that company's pre-existing data — new
// companies are seeded straight from the (already up-to-date) DEFAULT_TEMPLATES above.
async function applyScheduleTemplateSpecV2(db) {
  const rows = await db.all("SELECT * FROM shift_templates WHERE company_id = 'tamra'", []);
  ...
  async function ensureTemplate(t) {
    ...
    await db.run(
      "INSERT INTO shift_templates (id, company_id, role_id, label, start_time, end_time, needed, days, active, auto_assign, required_gender, allow_extra) VALUES (?,'tamra',?,?,?,?,?,?,1,?,?,?)",
...
// Scoped to 'tamra' — see the note on applyScheduleTemplateSpecV2 above.
async function applyScheduleTemplateSpecV3(db) {
  const row = await db.get("SELECT id, required_gender FROM shift_templates WHERE company_id = 'tamra' AND role_id = 'store' AND label = 'לילה חנות'", []);
...
async function applyScheduleTemplateSpecV4(db) {
  const row = await db.get("SELECT id, auto_assign, allow_extra FROM shift_templates WHERE company_id = 'tamra' AND role_id = 'store' AND label = 'בוקר חנות'", []);
...
async function deactivateOfficeTemplates(db) {
  await db.run("UPDATE shift_templates SET active = 0 WHERE company_id = 'tamra' AND role_id = 'office' AND active = 1", []);
}
...
// Scoped to 'tamra' — the only company with notifications old enough to need this.
async function cleanupVerboseUnderstaffedNotifications(db) {
  ...
  const rows = await db.all("SELECT id, text FROM notifications WHERE company_id = 'tamra' AND type = 'understaffed'", []);
...
async function dedupeScheduleStatusNotifications(db) {
  const rows = await db.all(
    "SELECT id, text FROM notifications WHERE company_id = 'tamra' AND audience = 'manager' AND type IN ('generated','understaffed') ORDER BY created_at ASC",
    []
  );
  const byWeek = {};
  rows.forEach(row => {
...
// Every method below is scoped to one company, bound at creation time (defaults to 'tamra' so
// every pre-existing call site — and every existing test — keeps working against the original
// company unchanged). lib/server.js creates one of these per request, per the company resolved
// from the URL/session (see companyIdFromRequest / session.companyId there).
function makeStore(db, companyId) {
  ...
  return {
    ...
    async getSettings() {
      const row = await db.get('SELECT data FROM settings WHERE company_id = ?', [cid]);
      return row ? JSON.parse(row.data) : Object.assign({}, DEFAULT_SETTINGS);
    },
    async updateSettings(patch) {
      const cur = await this.getSettings();
      const next = Object.assign({}, cur, patch);
      await db.run('UPDATE settings SET data = ? WHERE company_id = ?', [JSON.stringify(next), cid]);
      return next;
    },

    async listEmployees({ includePin } = {}) {
      const rows = await db.all('SELECT * FROM employees WHERE company_id = ? ORDER BY created_at ASC', [cid]);
      return rows.map(r => rowToEmployee(r, includePin));
    },
    async getEmployee(id, { includePin } = {}) {
      const row = await db.get('SELECT * FROM employees WHERE id = ? AND company_id = ?', [id, cid]);
    ...
    async updateEmployee(id, patch) {
      const cur = await db.get('SELECT * FROM employees WHERE id = ? AND company_id = ?', [id, cid]);
      if (!cur) return null;
      const next = {
    ...
    async listShiftTemplates() {
      const rows = await db.all('SELECT * FROM shift_templates WHERE company_id = ?', [cid]);
      return rows.map(rowToTemplate);
    },
    async createShiftTemplate(t) {
      const id = uid();
      await db.run('INSERT INTO shift_templates (id, company_id, role_id, label, start_time, end_time, needed, days, active, auto_assign, required_gender, allow_extra) VALUES (?,?,?,?,?,?,?,?,1,?,?,?)',
        [id, cid, t.roleId, t.label, t.start, t.end, t.needed, JSON.stringify(t.days), t.autoAssign === false ? 0 : 1, t.requiredGender || null, t.allowExtra ? 1 : 0]);
      return id;
    },
    async updateShiftTemplate(id, patch) {
      const cur = await db.get('SELECT * FROM shift_templates WHERE id = ? AND company_id = ?', [id, cid]);
      if (!cur) return null;
      const next = {
        role_id: patch.roleId != null ? patch.roleId : cur.role_id,
      ...
      await db.run('UPDATE shift_templates SET role_id=?, label=?, start_time=?, end_time=?, needed=?, days=?, active=?, auto_assign=?, required_gender=?, allow_extra=? WHERE id=? AND company_id=?',
        [next.role_id, next.label, next.start_time, next.end_time, next.needed, next.days, next.active, next.auto_assign, next.required_gender, next.allow_extra, id, cid]);
      return true;
    },

    async getScheduleWeek(weekStart) {
      const row = await db.get('SELECT * FROM schedules WHERE company_id = ? AND week_start = ?', [cid, weekStart]);
      const assignments = await db.all('SELECT * FROM assignments WHERE company_id = ? AND week_start = ?', [cid, weekStart]);
      ...
      const templateRows = await db.all('SELECT id, active, role_id FROM shift_templates WHERE company_id = ?', [cid]);
      const activeById = {};
      const roleById = {};
      ...
      const employeeRows = await db.all('SELECT id, is_senior, role_id, created_at FROM employees WHERE company_id = ?', [cid]);
      const seniorById = {};
    ...
    async getAssignment(id) {
      const row = await db.get('SELECT * FROM assignments WHERE id = ? AND company_id = ?', [id, cid]);
      return row ? rowToAssignment(row) : null;
    },
    async setAssignmentNoShow(id, noShow) {
      await db.run('UPDATE assignments SET no_show = ? WHERE id = ? AND company_id = ?', [noShow ? 1 : 0, id, cid]);
    },
    async listAssignmentsInRange(fromDate, toDate) {
      const rows = await db.all('SELECT * FROM assignments WHERE company_id = ? AND date >= ? AND date <= ? ORDER BY date ASC', [cid, fromDate, toDate]);
      return rows.map(rowToAssignment);
    },
    async listAssignmentsForEmployee(employeeId, fromDate) {
      const rows = await db.all('SELECT * FROM assignments WHERE company_id = ? AND employee_id = ? AND date >= ? ORDER BY date ASC', [cid, employeeId, fromDate]);
      return rows.map(rowToAssignment);
    },

    async listConstraints(employeeId) {
      const rows = employeeId
        ? await db.all('SELECT * FROM constraints WHERE company_id = ? AND employee_id = ? ORDER BY created_at DESC', [cid, employeeId])
        : await db.all('SELECT * FROM constraints WHERE company_id = ? ORDER BY created_at DESC', [cid]);
      return rows.map(rowToConstraint);
    },
    async addConstraint(c) {
      const id = uid();
      await db.run('INSERT INTO constraints (id, company_id, employee_id, kind, date, day_of_week, all_day, start_time, end_time, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)',
        [id, cid, c.employeeId, c.kind, c.date || null, c.dayOfWeek == null ? null : c.dayOfWeek, c.allDay ? 1 : 0, c.start || null, c.end || null, Date.now()]);
      return id;
    },
    async deleteConstraint(id, employeeId) {
      if (employeeId) return db.run('DELETE FROM constraints WHERE id = ? AND employee_id = ? AND company_id = ?', [id, employeeId, cid]);
      return db.run('DELETE FROM constraints WHERE id = ? AND company_id = ?', [id, cid]);
    },
    ...
    async listAvailability({ employeeId, fromDate, toDate } = {}) {
      let sql = 'SELECT * FROM availability WHERE company_id = ?';
      const args = [cid];
    ...
    async setWeekAvailability(employeeId, days) {
      for (const d of days) {
        await db.run('DELETE FROM availability WHERE company_id = ? AND employee_id = ? AND date = ?', [cid, employeeId, d.date]);
        await db.run('INSERT INTO availability (id, company_id, employee_id, date, choice, created_at) VALUES (?,?,?,?,?,?)',
          [uid(), cid, employeeId, d.date, d.choice, Date.now()]);
      }
    },
    ...
    async createSwapRequest(s) {
      ...
      await db.run('INSERT INTO swap_requests (id, company_id, assignment_id, requester_id, role_id, kind, status, claimed_by, date, shift_template_id, created_at, resolved_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,NULL)',
        [id, cid, s.assignmentId, s.requesterId, s.roleId, s.kind, 'open', null, assignment ? assignment.date : null, assignment ? assignment.shiftTemplateId : null, Date.now()]);
      return id;
    },
    async listSwapRequests({ status } = {}) {
      const rows = status
        ? await db.all('SELECT * FROM swap_requests WHERE company_id = ? AND status = ? ORDER BY created_at DESC', [cid, status])
        : await db.all('SELECT * FROM swap_requests WHERE company_id = ? ORDER BY created_at DESC', [cid]);
      return rows.map(rowToSwap);
    },
    async getSwapRequest(id) {
      const row = await db.get('SELECT * FROM swap_requests WHERE id = ? AND company_id = ?', [id, cid]);
    ...
    async updateSwapRequest(id, patch) {
      ...
      await db.run('UPDATE swap_requests SET status=?, claimed_by=?, resolved_at=? WHERE id=? AND company_id=?', [next.status, next.claimed_by, next.resolved_at, id, cid]);
    ...
    async addNotification(n) {
      const id = uid();
      await db.run('INSERT INTO notifications (id, company_id, audience, employee_id, type, text, severity, related_id, channels, read, created_at) VALUES (?,?,?,?,?,?,?,?,?,0,?)',
        [id, cid, n.audience, n.employeeId || null, n.type, n.text, n.severity || 'info', n.relatedId || null, JSON.stringify(n.channels || ['inapp']), Date.now()]);
      return id;
    },
    ...
    async replaceScheduleStatusNotification(weekStart) {
      await db.run("DELETE FROM notifications WHERE company_id = ? AND audience = 'manager' AND type IN ('generated','understaffed') AND related_id = ?", [cid, weekStart]);
    },
    async listNotifications({ audience, employeeId, limit } = {}) {
      let rows;
      if (audience === 'employee' && employeeId) {
        rows = await db.all('SELECT * FROM notifications WHERE company_id = ? AND audience = ? AND employee_id = ? ORDER BY created_at DESC LIMIT ?', [cid, 'employee', employeeId, limit || 100]);
      } else if (audience) {
        rows = await db.all('SELECT * FROM notifications WHERE company_id = ? AND audience = ? ORDER BY created_at DESC LIMIT ?', [cid, audience, limit || 100]);
      } else {
        rows = await db.all('SELECT * FROM notifications WHERE company_id = ? ORDER BY created_at DESC LIMIT ?', [cid, limit || 100]);
      }
    ...
    // duplicates that would each get a push. The endpoint lookup is intentionally NOT scoped to
    // this company: a browser/device has one push endpoint regardless of which company's page
    // registered it, and re-subscribing should move that single row to whichever company the
    // subscribing page currently belongs to rather than creating a second row.
    async savePushSubscription(sub) {
      const existing = await db.get('SELECT id FROM push_subscriptions WHERE endpoint = ?', [sub.endpoint]);
      if (existing) {
        await db.run('UPDATE push_subscriptions SET p256dh = ?, auth = ?, subject_type = ?, subject_id = ?, company_id = ? WHERE endpoint = ?',
          [sub.p256dh, sub.auth, sub.subjectType, sub.subjectId || null, cid, sub.endpoint]);
        return existing.id;
      }
      const id = uid();
    ...
    async listPushSubscriptions() {
      return db.all('SELECT * FROM push_subscriptions WHERE company_id = ?', [cid]);
    },
  };
}

// ---- company registry — not scoped to any one company, used to resolve which company a
// request belongs to (lib/server.js) and to loop over all companies (the cron endpoint). ----
async function getCompanyBySlug(db, slug) {
  const row = await db.get('SELECT * FROM companies WHERE slug = ?', [slug]);
...
module.exports = {
  initSchema, makeStore, DEFAULT_SETTINGS, DEFAULT_TEMPLATES, DEFAULT_COMPANIES, uid,
  getCompanyBySlug, getCompanyById, listCompanies,
};
